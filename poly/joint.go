...
func (p *PolyInfo) MarshalSize() int {
	return util.UInt64Size(uint64(p.N)) +
		util.UInt64Size(uint64(p.R)) +
		util.UInt64Size(uint64(p.T))
}
...
func (p *PolyInfo) MarshalBinary() ([]byte, error) {
	buf := make([]byte, 0)
	if bt, err := util.UInt64ToBuf(uint64(p.T)); err != nil {
		return nil, errors.New(fmt.Sprintf("Error while marshaling T : %v", err))
	} else {
		buf = append(buf, bt...)
	}

	if br, err := util.UInt64ToBuf(uint64(p.R)); err != nil {
		return nil, errors.New(fmt.Sprintf("Error while marshaling R : %v", err))
	} else {
		buf = append(buf, br...)
	}

	if bn, err := util.UInt64ToBuf(uint64(p.N)); err != nil {
...
func (p *PolyInfo) UnmarshalBinary(buf []byte) error {
	t, err := util.BufToUInt64(buf)
	if err != nil {
		return errors.New(fmt.Sprintf("Error unmarshaling T : %v", err))
	}
	r, err := util.BufToUInt64(buf[util.UInt64Size(uint64(t)):])
	if err != nil {
		return errors.New(fmt.Sprintf("Error unmarshaling R : %v", err))
	}
	n, err := util.BufToUInt64(buf[util.UInt64Size(uint64(t))+util.UInt64Size(uint64(r)):])
	if err != nil {
